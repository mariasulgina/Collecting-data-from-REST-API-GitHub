...
2. В корне проекта найдите файл `appsettings.example.json`, при заполнении `appsettings.json` обращайте внимание на него как на пример.
...
### 5.3. Описание связей (Relationships)
...
1. Пользователь выбирает пункт 2 и вводит логин `octocat`.
2. Приложение загружает JSON, мапит его в `GitHubUserEntity` и сохраняет в `github_lab.db`.
3. Пользователь выбирает пункт 4 и вводит `https://github.com`.
4. Парсер извлекает заголовок "GitHub: Let’s build from here" и список ссылок, сохраняя их в MongoDB.
5. Результаты проверяются через пункты 3 и 5 меню.
